// Command netfulfil-reports is the READER composition root of the
...
// pool and serves the acknowledgement report and its freshness over
// REST. It writes nothing: the writer (cmd/netfulfil-projector) is a
// separate deployable and owns the schema.
//
...
func main() {
	if err := run(); err != nil {
		slog.Error("netfulfil-reports exited with error", "error", err)
		os.Exit(1)
	}
}
