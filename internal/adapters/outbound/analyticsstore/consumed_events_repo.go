...
// ConsumedEventsRepo is a pgxpool-backed idempotency gate over the
// analytical database's analytics_consumed_events table. It is the
// consumer's dedupe set, kept separate from the projection's own event-id
// claim (analytics_processed_events) so the two idempotency layers do not
